...
> 2026-09-15追記: この文書は過去の作業記録です。現在の公開状態、Macの開発手順、Cloudflare試験版は [CURRENT_STATUS.md](CURRENT_STATUS.md)、[README.md](README.md)、[CLOUDFLARE.md](CLOUDFLARE.md) を優先してください。以下の「非公開」「作業ブランチ」等には古い情報が含まれます。

最終更新: 2026-08-05（ヒーローバグ修正とモバイル写真の全面刷新。§12 を参照）
...
- `public/hero-members.jpg`: 実写のヒーロー写真（横長、PC版のヒーロー背景で使用）
- `public/hero-mobile.png`: 実写のヒーロー写真（縦長、モバイル版のヒーロー背景専用。§12参照。2026-08-05にユーザー提供の別カットへ差し替え済み）
- これらは絶対に元のプレースホルダー（Gemini生成の3D風イラスト`hero.png`や自作SVGアイコン）に戻さないこと

## 4. コンテンツの状態

現時点で残っているコンテンツ未確定のTODOは無し。

- **裏方メンバー（あしゅ・ふらとら・サイヨーマ）の自己紹介文**: Notionの「スタッフ」データベースに実際の紹介文が入力済みで、本番サイトには正式な内容が表示されている（`src/app/page.tsx`実機確認済み、2026-08-05）。`src/app/page.tsx:111`付近の`staffMembers`（Notion未接続時のフォールバック用ハードコード）に「詳しいプロフィールは準備中です」というプレースホルダーが残っているのは意図的な仕様で、TODOではない
...
## 12. 2026-08-05 セッション: ヒーローバグ修正とモバイル写真の全面刷新

前回(§11)の直後に発覚したバグ2件の修正から始まり、実機フィードバックを繰り返し反映しながらモバイルヒーローを最終的に**PC版と同じ「写真1枚にテキストを全部重ねる」構成**まで作り直した。

### 12.1 バグ修正（§11の副作用）
- **タイトルが写真の下に隠れる**: §11でDOM順を「テキスト→写真」に変えた際、z-index未指定のためsm以上で写真(opacity 0.9)がタイトルより手前に描画されていた。テキスト側に`z-10`を追加して解消
- **バッジ(ステッカー)がタイトルと重なる**: 「総再生12億回+」「最終1位14回」の位置をタイトルと衝突しないよう再調整

### 12.2 モバイル写真の刷新（複数イテレーション）
ユーザーから縦長の実写カットが2枚提供され、最終的に`public/hero-mobile.png`として採用（旧カットは削除済み）。試行の過程:

1. 写真先行+タイトル重ねのDOM順に変更（`aspect-[5/4]`→`aspect-[3/4]`→`aspect-[4/5]`→`aspect-[7/9]`→最終`aspect-[8/13]`と縦横比を調整しながら「顔が見切れない」「縦長を活かす」の両立点を探った）
2. `HeroPhoto`に`zoom`（中央基準の追加ズーム）と`parallax`（GSAP ScrollTriggerでのスクロール連動視差）のpropsを追加
3. 実機(iPhone)スクショで「SCROLLが見えない」「上端の境目が硬い」「下に白い空白が残る」との指摘を受け、都度調整
4. **最終形**: ユーザーが手描きラフを提示（「写真いっぱいの背景+タイトル+scrollを写真上に重ねる」構成）。「レイアウトのみの参考」と確認した上で実装。モバイルは写真を`aspect-[8/13]`(横縦比1.615、写真の実アスペクト比1.792に対し十分安全マージンあり)でセクション全体に敷き、タイトル・タグライン・Scrollインジケーターをすべて写真内に絶対配置で重ねる（PC版と同じ考え方に統一）。写真下端はh-[48%]の暗いグラデーションスクリムで白文字の可読性を確保
5. ヒーロー直後に唐突な白背景が来て間延びして見える問題を、`bg-gradient-to-b from-black/70 to-white`の静的グラデーション帯(`h-20`, `sm:hidden`)で解消

### 12.3 顔が見切れないための考え方（今後も踏襲すること）
`object-cover`は、コンテナの縦横比(h/w)が画像の縦横比(h/w)より**小さい**（＝コンテナの方が横長）場合は上下だけをクロップし、**大きい**（縦長）場合は左右をクロップする。今回の写真は実アスペクト比が高さ/幅=1.792なので、コンテナのアスペクト比をそれより低く保つ限り、どれだけ高さを伸ばしても左右（＝顔）は絶対にクロップされない。逆に言うと、コンテナをこの値以上に縦長にした瞬間に顔が切れ始める。新しい写真に差し替える・比率を変える際は、まず`naturalWidth/naturalHeight`比を確認し、コンテナのaspect比がそれを超えないようにすること。

### 12.4 SectionBlendは静的な色つなぎには使えない（重要な学び）
セクション間の色を繋ぐ際、既存の`SectionBlend`（§8.2参考、GSAP ScrollTriggerで背景色をスクロール量に応じて補間する部品）を流用しようとしたが、これは「帯をスクロールで通過する間に色が変わる」**時間軸**の演出であり、初回表示時（スクロール前）は単色にしか見えない。「最初から視覚的に繋がって見える」ことが目的の場合は使えないので、素直な静的CSSグラデーション(`bg-gradient-to-b from-X to-Y`)を使うこと。

### 12.5 このリポジトリ固有のTailwindバグ（重要・要注意）
`-mt-20`や`-mt-24`など、特定の負のマージン標準ユーティリティクラスが**一切CSSとして生成されず**、`getComputedStyle().marginTop`が`0px`のままになる現象を確認した（`-mt-28`/`-mt-32`など他の値は正常に生成される。開発サーバーの再起動でも直らなかった）。原因は不明。**負のマージンで見た目が変わらない場合は、まず`getComputedStyle`で実際に値が当たっているか確認すること**。再現を避けるため、今回調整したものは`-mt-[96px]`のような角括弧の任意値記法に置き換えて回避した。今後この種の値を追加する際も任意値記法を優先すること。

### 12.6 検証方法（このセッションで確立した手順）
- 320 / 375 / 639 / 640 / 1280pxに加え、**実機のセーフエリアを想定した375×660**でも必ず確認する（Safariのアドレスバー分、実際の可視高さは`window.innerHeight`の指定値より小さくなることが多い）
- 顔の見切れ確認は`imgEl.getBoundingClientRect().width`とコンテナ幅を比較し、クロップ率を数値で出す（目視だけでなく）
- 要素の重なりは`getBoundingClientRect()`同士のAABB判定関数を都度書いて機械的に確認する（`overlapsTitle`のような即席関数で十分）
- ユーザーが手描きラフや外部画像を送ってきた場合、「これは実際にこう見せたいのか、それともレイアウトの参考なのか」を先に確認してから着手する（今回は`AskUserQuestion`で確認し、手戻りを防げた）
